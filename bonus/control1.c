...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/16 14:06:03 by muguveli          #+#    #+#             */
/*   Updated: 2024/03/18 15:09:29 by beyarsla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	copy_map(t_game *game)
{
	...
	if (result == 2)
		...
		ft_exit(_ENEMY_REACHABLE, "Enemy not reachable", game);
}
...
static void	ft_frame_control(t_game *game)
{
	...
	while (++y < game->map->map_y)
		if ((game->map->game_map[y][0] != _WALL
			...
			- 1] != _WALL))
			ft_exit(_FRAME_ERR, "The borders of the map have to be a wall",
				game);
...
static void	ft_get_count(char c, t_game *game)
{
	if (c == _PLAYER && game->counters->p_counter++)
		if (game->counters->p_counter > 1 || game->counters->p_counter == 0)
			ft_exit(_PLAYER_COUNT, "There must be one player in the game",
				game);
	...
	while (++y < game->map->map_y)
	{
		...
		while (++x < game->map->map_x)
		{
			if (ft_strchr(_ELEMENTS, game->map->game_map[y][x]) == 0)
				ft_exit(_CHAR_ERR, "Unrecognized character in map file", game);
